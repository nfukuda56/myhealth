...
function renderPacePanel(stats, balanceData, allDates) {
  ...
  el.innerHTML = `
    <div style="display:flex;flex-direction:column;align-items:center;gap:4px;padding:8px 10px">
      ...
      <div style="font-family:var(--mono);font-size:10px;color:var(--text-muted)">
        週 <span style="color:var(--text)">${fmt(pace)} kg</span> ／ 月 <span style="color:var(--text)">${fmt(month)} kg</span>
      </div>
    ...
    <div style="border-top:1px solid var(--border);padding:7px 10px;font-family:var(--mono);font-size:10px;color:var(--text-muted);display:grid;grid-template-columns:1fr 1fr;gap:4px 6px">
      <div>平均摂取<br><span style="color:var(--text)">${fmtk(avgIntake)}</span></div>
      <div>運動消費<br><span style="color:var(--text)">${fmtk(avgBurned)}</span></div>
      <div style="grid-column:1/-1">平均収支<br><span style="color:${bColor}">${fmtb(avgBalance)}</span></div>
    </div>
  `
}
